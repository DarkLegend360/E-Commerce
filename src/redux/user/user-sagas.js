...
import {auth,createUserAccount,googleProvider,getCurrentUser} from "../../firebase/firebase.util";
import { signInSuccess, signInFailure, signOutFailure, signOutSuccess, signUpFailure, signUpSuccess} from "./user-actions";

function* getSnapshotFromUserAth(user,additionalData) {
    try {
...
export function* signInWithGoogle() {
    try {
        ...
        yield getSnapshotFromUserAth(user);
    } catch(error) {
        yield put(signInFailure(error));
    }
}
...
export function* isUserAuthenticated() {
    try{
        ...
        if(!userAuth) return;
        yield getSnapshotFromUserAth(userAuth);
    }catch(error) {
...
export function* signUp({payload:{displayName,email,pass}}) {
    try{
...
export function* signInAfterSignUp({payload:{user, additionalData}}) {
    yield getSnapshotFromUserAth(user,additionalData);
}
